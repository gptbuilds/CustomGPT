...
# Function to manage database for SMS threads
...
def handle_sms():
    ...
    try:
        ...
        return jsonify({"error": "Assistant ID not found or error in assistant.json"}), 500

    if not assistant_id:
        return jsonify({"error": "Assistant ID not found in assistant.json"}), 404

    # Parse the incoming data from Twilio
    user_input = request.form.get('Body', '')
    phone_number = request.form.get('From', '')

    if not user_input or not phone_number:
        print("Error: SMS message or phone number not provided.")
        return Response("Invalid request", status=400)

    # Retrieve or create a new thread_id for the phone_number
    thread_id = manage_sms_database(phone_number)
    if not thread_id:
        thread_id = generate_new_thread_id()
        manage_sms_database(phone_number, thread_id)

    # Send the incoming SMS to the OpenAI Assistant
    try:
        client.beta.threads.messages.create(thread_id=thread_id, role="user", content=user_input)
        run = client.beta.threads.runs.create(thread_id=thread_id, assistant_id=assistant_id)
    except Exception as e:
        print(f"Error interacting with OpenAI Assistant: {e}")
        return jsonify({"error": "Failed to process message with OpenAI Assistant"}), 500
      ...
      if run_status.status == 'completed':
          ...
          for action in required_actions["tool_calls"]:
              ...
              if func_name == "query_real_estate":
                ...
                # Define keywords that indicate the need to send an SMS
    ...
    try:
        phone_number = +61481737005
        response_message = client_twilio.messages.create(
            
            body=response,
            from_=os.getenv("TWILIO_PHONE_NUMBER"),
            to=phone_number
        )
        print(f"SMS response sent with SID: {response_message.sid}")
    except Exception as e:
        print(f"Error sending response SMS via Twilio: {e}")
        return jsonify({"error": "Failed to send SMS response via Twilio"}), 500
